...
- Use tiered models: `model: "sonnet"` for data-gathering, `model: "opus"` for gap analysis, synthesis, and skeptic. All agents MUST specify `subagent_type: "deep-researcher"` in every Agent tool call — this is the structural enforcement that prevents sub-agents from spawning further agents. Never use a general-purpose subagent type.
